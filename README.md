...
### 4.3.0 (2026)
- **版本更新**: 插件版本升级到 4.3.0
- **Marmoset烘焙模式**: 将原共享贴图流程重命名为“多对多烘焙”，保留多组同名 `_high/_low` 共用一套低模UV和一套贴图的原有工作流
- **多对一烘焙**: 新增真正的多个高模对应一套低模流程，导出前临时合并高模并统一 `_high/_low` 命名，减少 Toolbag 自动分组错误导致的黑图问题
- **贴图回填修复**: 优化共享烘焙贴图回填逻辑，按本次烘焙项目名精确查找输出贴图，避免 Albedo/Normal 错误引用旧贴图或无关模型贴图
- **低模拓扑**: 新增“类ZBrush低模拓扑”流程，结合 Quad Remesher、Voxel Remesh、Shrinkwrap、细分和最终减面，辅助从高模生成更稳定的低模
- **贴图资源防丢失**: 新增独立贴图候选选择工具，优化资源贴图收拢、智能查找丢失贴图、重名贴图冲突处理和 textures 目录优先级
- **稳定性**: 贴图扫描扩展到材质、世界和合成器节点，跳过打包贴图与非文件贴图，并补充重连失败提示和保存提醒
...
- **安全加密**: API密钥采用加密存储，支持主密码保护
- **用户体验**: 优化翻译界面，提供实时翻译预览和错误提示
- **多语言支持**: 支持中英文互译，适配游戏开发命名规范

### 3.3.10 (2025)
- **UI优化**: 优化了UI布局，将海岛配方道具智能重命名模块移至角色重命名模块上方
- **用户体验**: 改进了功能模块的排列顺序，提升操作流畅度和逻辑性

### 3.3.9 (2025)
- **Bug修复**: 修复SDK读取卡住问题，新增手动检测SDK按钮提升用户体验
- **新增功能**: 角色重命名模块新增"命名选中道具"功能，支持mesh_tool_体型_序号后缀命名规则
- **智能后缀**: 道具命名优先使用贴图后缀，提供完整的输入验证和错误提示

### 3.3.8 (2025)
- **工作流优化**: 加入大量表格，点击直接跳转，提高同步效率
- **用户体验**: 优化UI面板布局，提升操作流畅度
- **代码优化**: - 重构代码结构，提升运行性能,修复已知问题和错误

### 3.3.6 (2025)
- **Bug修复**: 修复纹理工具中"贴图后缀"和"后缀"输入框同步输入的问题
- **UI改进**: 将"贴图后缀"输入框独立为单独的属性，实现独立输入控制
- **用户体验**: 提升纹理重命名工具的使用体验，避免输入框混淆
